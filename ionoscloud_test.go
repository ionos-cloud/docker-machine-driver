...
import (
	...
	mockutils "github.com/ionos-cloud/rancher-driver/utils/mocks"
...
func TestDriver_CreateServerErr(t *testing.T) {
	...
	clientMock.EXPECT().GetDatacenter(driver.DatacenterId).Return(dc, nil)
	clientMock.EXPECT().CreateLan(driver.DatacenterId, driver.MachineName, true).Return(lan, nil)
	clientMock.EXPECT().CreateServer(driver.DatacenterId, driver.MachineName, driver.CpuFamily, driver.ServerAvailabilityZone, int32(driver.Ram), int32(driver.Cores)).Return(server, fmt.Errorf("error"))
	err = driver.Create()
	assert.Error(t, err)
}
